...
description: >
  Database harmonization between CellChatDB.human(v2.2.0) and CellPhoneDB (V5.0)
vignette: >
  %\VignetteIndexEntry{Database harmonization by scpeakeR - Part 1}
...
CC.int %>% select(interaction_name, pathway_name, ligand_name, ligand, receptor_name, receptor) %>% head()
```
...
We observed duplicated complex keys after sorting subunits in CPDB:
```{r}
dups <- CPDB.int %>% 
  count(ligand, receptor) %>% 
  filter(n > 1) %>%  
...
Apart from internally standardizing the order of protein subunits within complexes, the order of the interaction partners affected the number of overlapping interactions as well. Since we defined interactions as ligand|receptor pair, we identified cases where interactions in **CCDB** were arranged as "ligandA|receptorB", while the same interaction was arranged as "receptorB|ligandA" in **CPDB**. This is often the case for cell-contact interactions, where directionality is inconsequential. 
...
# STEP6: Addressing partial matches
...
$$ Jaccard Index = (number of identical subunits) / (total number of shared subunits) $$
Suppose we have a complexes made up of subunitA_subunitB in CCDB and another similar complexes made up of subunitA_subunitB_subunitC. The pairwise jaccard index for these two complexes will be 0.667 (3 d.p.) since they both share subunits A and B, and the number of unique subunits is 3 (subunits A, B and C).
...
To customize CCDB for cell-cell communication analysis with CellChat, we refer to the document attached [here](https://rdrr.io/github/sqjin/CellChat/f/tutorial/Update-CellChatDB.Rmdhttps://rdrr.io/github/sqjin/CellChat/f/tutorial/Update-CellChatDB.Rmd).
...
## Extract interaction partners of simple genes and add new changes to geneInfo slot
...
To customize CPDB with our revised changes, we refer to the document [here](https://pypi.org/project/cellphonedb/).
...
## Customizing database with changes implemented
...
# Saved customized CellPhoneDB .zip file to  CCCtools GitHub 
...
# Final touches to harmonized databases
...
```{r, eval = FALSE}
## Save CCDB and CPDB as .rda to CCCtools GitHub
save(CCDB, file = "data/CCDB.rda")
save(CPDB, file = "data/CPDB.rda")
```

This concludes the process performed in **Part 1** of the Database Harmonization conducted, as well as the customization of the respective databases to be run with CellChat and CellPhoneDB respectively. In **Part 2**, we outline the full convergence of the harmonized databases, providing users with a single merged database comprising of interactions from both CellChat and CellPhoneDB. You may refer to **Part 2** [here]().
