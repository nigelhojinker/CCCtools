---
title: "Database harmonisation by scpeaker - Part 1"
author: "Ho Jin Ker Nigel, Adaikalavan Ramasamy, Gokce Oguz"
description: >
  Database harmonisation between CellChatDB.human(v2.2.0) and CellPhoneDB (V5.0.1)
date: 2025/12/30
format: 
  html:
    toc: true
...
In this document, we outline the database harmonisation steps taken to determine the degree of overlap between CellChat and CellPhoneDB, which are listed as:
...
After Step 1 of Database harmonisation, we observed that there were only 585 overlapping interactions across CCDB and CPDB. As expected, no complex interaction partners across both databases overlapped since CPDB complexes were not converted to Uniprot IDs in this step.
...
CC.int %>% 
  filter(! complex_ligand, ! ligand %in% uniprotkb$entry) %>%
  pull(ligand, ligand_name)     # ligand:   CCL3L1, CCL3L3, IFNA1, IFNA13 invalid
# CCL3L1   CCL3L3   CCL3L1    IFNA1 
#     NA       NA       NA "L0N195" 
CC.int %>% 
  filter(! complex_receptor, ! receptor %in% uniprotkb$entry) %>% 
  pull(receptor, receptor_name) # receptor: GPR1 missing an entry
# GPR1 
#   NA 
```
...
CPDB.int %>% 
  filter(! complex_ligand, ! ligand %in% uniprotkb$entry) %>%
  pull(ligand)   # ligand: HLAA, HLAB, HLAC, IFNA1 not uniprot ID format 
# [1] "HLAA"  "HLAB"  "HLAC"  "HLAC"  "IFNA1"
CPDB.int %>% 
  filter(! complex_receptor, ! receptor %in% uniprotkb$entry) %>% 
  pull(receptor) # receptor: no invalid uniprot IDs
# character(0)
```
...
After Step 2 of Database harmonisation, we observed that there was a slight increase in the number of overlapping interactions (585 to 591) across CCDB and CPDB.
...
This concludes the process performed in **Part 1** of the Database harmonisation conducted. In **Part 2**, we outline the customization of the CellChatDB and CellPhoneDB with the proposed changes above. You may refer to **Part 2** [here](https://nigelhojinker.github.io/scpeaker/data-raw/Database_harmonisation_part2.html).
